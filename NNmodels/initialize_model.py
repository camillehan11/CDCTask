...
class MTL_Model(object):
    ...
    def exp_lr_sheduler(self, epoch):
        ...
        for param_group in self.optimizer.param_groups:
            param_group['lr'] *= self.lr_decay_epoch
            return None
    ...
    def optimize_model(self, input_batch, label_batch):
        model_server = deepcopy(self.shared_layers)
        ...
        for w, w_t in zip(model.parameters(), model_server.parameters()):
            w_diff.append((torch.pow(torch.norm(w.data - w_t.data), 2)).cpu().numpy())
        return batch_loss.item(), w_diff

    def optimize_proxmodel(self, args,input_batch, label_batch):
        model_server = deepcopy(self.shared_layers)
        self.shared_layers.train(True)
        if self.specific_layers:
            self.specific_layers.train(True)
        ...
        model = self.shared_layers
        model.zero_grad()
        w_diff = torch.tensor(0.,device='cuda:0')
        for w, w_t in zip(model.parameters(), model_server.parameters()):
            w_diff += torch.pow(torch.norm(w.data - w_t.data), 2)
            # w.grad.data += self.args.mu * (w.data - w_t.data)
            w.grad.data += args.mu * (w_t.data - w.data)
        batch_loss += args.mu / 2. * w_diff
