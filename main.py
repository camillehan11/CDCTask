...
def get_TGserver_class(args, TGservers, TCclients):
    ...
    print(f'class distribution among TGserver {TGserver_class}')


def initialize_TGservers_iid(num_TGservers, TCclients, args, TCclient_class_dis):
    """
    This function is specially designed for partiion for 10*L users, 1-class per user, but the distribution among TGservers is iid,
    10 TCclients per TGserver, each TGserver have 10 classes
    :param num_TGservers: L
    :param TCclients:
    :param args:
    :return:
    """
    # only assign first (num_TGservers - 1), neglect the last 1, choose the left
    TGservers = []
    ...
    for eid in range(num_TGservers):
        ...
        TGservers.append(Server(id=eid,
                          cids=assigned_TCclients_idxes,
                          shared_layers=copy.deepcopy(TCclients[0].model.shared_layers)))
        ...
        p_TCclients[eid] = [sample / float(TGservers[eid].all_trainsample_num)
                          for sample in list(TGservers[eid].sample_registration.values())]
        TGservers[eid].refresh_TGserver()
    # And the last one, eid == num_TGservers -1
    eid = num_TGservers - 1
    assigned_TCclients_idxes = []
    for label in range(10):
...
# def initialize_TGservers_niid(num_TGservers, TCclients, args, TCclient_class_dis):
#     """
#     This function is specially designed for partiion for 10*L users, 1-class per user, but the distribution among TGservers is iid,
#     10 TCclients per TGserver, each TGserver have 5 classes
#     :param num_TGservers: L
#     :param TCclients:
#     :param args:
#     :return:
...
#                 #     5 labels in total
#                 if len(TCclient_class_dis[label]) > 0:
#                     assigned_TCclient_idx = np.random.choice(TCclient_class_dis[label], 1, replace=False)
#                     TCclient_class_dis[label] = list(set(TCclient_class_dis[label]) - set(assigned_TCclient_idx))
#                 else:
#                     label_backup = 2
#                     assigned_TCclient_idx = np.random.choice(TCclient_class_dis[label_backup], 1, replace=False)
#                     TCclient_class_dis[label_backup] = list(
#                         set(TCclient_class_dis[label_backup]) - set(assigned_TCclient_idx))
#                 for idx in assigned_TCclient_idx:
#                     assigned_TCclients_idxes.append(idx)
#         TGservers.append(Server(id=eid,
#                           cids=assigned_TCclients_idxes,
...
def CDCtask(args):
    ...
    TGservers = [[] for x in range(args.num_TGservers)]
    TCclients = [[] for x in range(args.num_TCclients)]
    batch = 50
    fd = 10
    Ts = 20e-3
    n_x = 1
    ...
    for n in range(args.num_TGservers):
        Agents_list.append(
            TD3_agent(state_dim=state_num,action_dim=action_num, max_action=action_num,batch_size=batch_size
                       ))
    ...
    for num_comm in tqdm(range(args.num_communication)):
        ...
        for num_TGserveragg in range(args.num_TGserver_aggregation):
            ...
            correct_all = 0.0
            total_all = 0.0
            selected_cids = [[] for x in range(args.num_TGservers)]
            cids = [[] for x in range(args.num_TGservers)]
            ...
            for i, TGserver in enumerate(TGservers):
                ...
                p_TCclients = [1 / args.num_TCclients for x in range(maxM)]
                ...
                for j in range(len(s_actor_agent)):
                    o = tg * (np.log(sigmoid(
                        (trate * treliability * tdelay)) + 1) ** 2)
                    # o =  (np.abs(c[j] - glob[i]))* (sigmoid(
                    #     (trate * treliability * tdelay)) + 1)

                    if trate[j] >= rr and treliability[j] >= rl and tdelay[j] <= rd and (np.abs(c[j] - glob[i])) <= 2.5:
                        s_actor_t[j] = s_actor_t[j]
                    else:
                        s_actor_t[j] = -1
                    rewardlist[j, i] = args.mu * (rd - tdelay[j]) / (5e2) + (1 - args.mu) * (o[j] / 20)
                ...
                execution_time_cost = execution_time_cost + time.time() - st
            ...
            s_actor_next, s_critic_next, reward_rate, sum_rate, rate, reliability, delay1, delay2, g, loss = env.step(p,
                                                                                                                      device,
                                                                                                                      R)
            delay = delay1 * 1e-2

            for i in range(args.num_TGservers):
                TGservers[i] = (Server(args, id=i, cids=selected_cids[i],
                                 shared_layers=copy.deepcopy(TCclients[0].model.shared_layers)))
        ...
        writer.add_scalar(f'delay',
                          mean_delay,
                          num_comm + 1)
        writer.add_scalar(f'gradient divergence',
                          mean_gradient * mean_acc,
                          num_comm + 1)
        writer.add_scalar(f'Mean_reward',
                          mean_reward * mean_acc,  # TCclient_cons += TCclient_con
                          num_comm + 1)
